...
#      ex) 계산기: 덧셈, 뺄셈, 나눗셈, 곱셈 
#          -> 각각이 개별 기능..1개보다 4개 만드는 게 효율 good
#          -> 제곱 기능(곱셈 2번)        
#          -> 곱셈()x 2번 사용 가능 -> 재사용성 Up
...
#    $ 함수 호출 후 함수 종료된 경우 "호출했던 곳"으로 돌아감
#    $ 입력 매개변수의 순서가 매우 중요
...
#    -> 종속성 : A 코드를 수정하면 B, C 코드도 수정해야 하는 경우
#. 5. Default parameter
#  + 함수 정의문과 호출문의 매개변수 갯수는 동일해야 함
#    -> 동일하지 않은 경우 Dufault parameter를 사용해서 기본값 할당 가능

#  + Default는 맨 뒤 인자부터 사용 가능! 
# def test(a, b, c=3):        #(O)
# def test(a, b=2, c=3):      #(O)
# def test(a=1, b=2, c=3):    #(O)
# def test(a=1, b, c=3):      #(X)
...
#    + 같은 이름의 지역변수와 전역변수가 존재하는 경우 가까운 곳이 우선순위가 높음

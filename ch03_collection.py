...
["a", "b", "c"]   # 리스트 Packing
a, b, c = ["a", "b", "c"]   # 리스트 Unpacking (C, JAVA엔 없음)

test = [1, 2, 3]
# 가. append() : 맨 마지막에 값 추가
test.append(4)
print(4)
# 나. insert() : 원하는 인덱스에 값 추가(새치기.. 뒤 녀석들 순번 밀림)
test.insert(1, 99) #1번 인덱스에 99 추가
print(test)
# 다. extend() : 리스트 병합
a = [1, 2]
b = [2, 3]
a.extend(b)      # a 기준으로 b 추가 = a+b
print(a)         # 출력
# 라. remove() : 값으로 삭제
test.remove(2)
print(test)
# 마. pop() : 인덱스로 삭제
test.pop(1)      # 리스트에서 제거되지만, 값을 따로 보관하고 싶은 경우.
print(test)
print(temp)
# 바. index() : 특정 값 인덱스 찾기
test.index(1)
...
#    + packing과 unpacking 가능
#    - () 사용, () 생략가능
#    - immutable(생성 후 변경 불가) -> 멤버 변수, 정렬 불가
#    * 함수 return 값 튜플!
...
d = (5)        # 튜플
e = 5          # 정수형(int)
e = 5,         # 원소 하나일 때 괄호 제거하는 법 : 나 원소값 하나지만 튜플이야~
print(type(e)) # 타입 확인
